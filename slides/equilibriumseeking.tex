...
\begin{tcolorbox}[enhanced, colframe=monokaiBG!33!white, colback=white, width=35em, rounded corners, drop shadow southwest=monokaiBG!33!white]
\vspace*{-1em}
\begin{tcolorbox}[frame empty, colback=white, width=28.5em]
	\textsc{Generalized feedback Nash equilibrium problem}
\end{tcolorbox} 
\vspace*{-1.66em}
\begin{equation*}
	\text{Find }~ K^{\star} = ( K^{p^{\star}} )_{p=1}^{N_P} ~\text{ such that }~ K^{\star} \in \mathbf{fix}(\mathrm{BR}^1 \times \cdots \times \mathrm{BR}^{N_P})
\end{equation*}
\separatorLine{0.33em}{0.75em}
\begin{center}
	-- The \emph{best-response map} $\mathrm{BR}^p(K^{-p})$ is the solution set of the problem --
\end{center}
\vskip0.5em
\begin{align*}
	\underset{K^p}{\text{minimize}} \quad
		& \mathrm{E}\sum_{n=0}^{\infty} \left\| \WeightMatrix \XU{}{[n]}{^p} \right\|_2^2 \\[0.5ex]
	\underset{(\forall n)}{\text{subject to} } \quad
		& \XUBig{}{}{} = \KInverse \NoiseMatrixBig w   \\[0.33ex]
		& \ConstMatrix \XU{}{[n]}{} \preceq 1, ~~~ K^p[n] \in \mathcal{S}^p[n] \\[-2.33ex]
\end{align*}
\end{tcolorbox}
